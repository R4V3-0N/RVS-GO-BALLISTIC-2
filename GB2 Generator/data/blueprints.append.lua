-- THE ENTIRETY OF MY LUAXP FORK BECAUSE WE DON'T HAVE REQUIRE YET
-- IT IS MODIFIED TO ALLOW '@' TO BE USED IN VARIABLE NAMES
...
AMMO_FILE_PATH = "/data/ammo.xml"
AMMO = "ammoBlueprint"
WEAPON = "weaponBlueprint"
USES_GB_AMMO = "usesGBAmmo"
BASE = "base"
LITERAL_TEXT = "literalText"
LITERAL_NODE = "literalNode"
CONDITION_TAG = "condition"
STATS_TAG = "stats"
CONDITION_ATTRS = {"nameEquals", "nameContains", "hasTag"}
CONDITION_ATTR_FUNCS = {
	["nameEquals"] = function(value, contextTable) return contextTable["@name"] == value end,
	["nameContains"] = function(value, contextTable) return string.find(contextTable["@name"], value) end,
	["hasTag"] = function(value, contextTable)
		local splitTags = value:gmatch("([^%.]+)")
		for _, tag in splitTags do
			if contextTable[tag] == nil then return false
			else contextTable = contextTable[tag] end
		end
		return true
	end
}
...
-- Any not here are handled in another file
SPECIAL_TAG_FUNCS = {
	["if"] = function(ifNode, generatedWeaponTable, ammoTable, contextTable)
		-- This shouldn't be necessary, but it is for some reason
		if getmetatable(ifNode.content.__contents) ~= NumberedKeyTable then
			print("This is necessary")
			setmetatable(ifNode.content.__contents, NumberedKeyTable)
		end
		
		if ifNode.content[CONDITION_TAG] == nil then
			error("if node has no " .. CONDITION_TAG .. " tag.")
		end
		
		if ifNode.content[STATS_TAG] == nil then
			error("if node has no " .. STATS_TAG .. " tag.")
		end
		
		local result, err = luaxp.evaluate(ifNode.content[CONDITION_TAG].content, contextTable)
		if result == nil then
			error("Condition \"" .. ifNode.content[CONDITION_TAG].content .. "\" failed to be evaluated with error: \"" .. err.message .. "\".")
		elseif result then
			evaluateAmmoStats(ifNode.content[STATS_TAG], contextTable)
			applyAmmoStats(generatedWeaponTable, ifNode.content[STATS_TAG], true)
		end
	end,
	["match"] = function(matchNode, generatedWeaponTable, ammoTable, contextTable)
		local hasConditionAttr = false
		for _, attr in ipairs(CONDITION_ATTRS) do
			if matchNode.attrs[attr] ~= nil then hasConditionAttr = true break end
		end
		if not hasConditionAttr then
			error("match node has no condition attributes.")
		end
		
		local result = true
		for attrName, attrValue in pairs(matchNode.attrs) do
			if CONDITION_ATTR_FUNCS[attrName] then
				result = result and CONDITION_ATTR_FUNCS[attrName](attrValue, contextTable)
			else
				print("Unrecognized attribute \"" .. attrName .. "\" in match node.")
			end
		end
		
		if result then
			evaluateAmmoStats(matchNode, contextTable)
			applyAmmoStats(generatedWeaponTable, matchNode, true)
		end
	end
}
...
ammoBlueprints = {mod.xml.parse(mod.vfs.pkg:read(AMMO_FILE_PATH))}

ammoWeaponBlueprints = {count = 0}
for element in document.root:children() do
	if element.name == WEAPON and element.attrs[USES_GB_AMMO] then
		ammoWeaponBlueprints.count = ammoWeaponBlueprints.count + 1
		ammoWeaponBlueprints[ammoWeaponBlueprints.count] = element
	end
end
...
Class = {
	...
	__newindex = function(tbl, _, _)
		error("Adding new fields to a class is not allowed.", 2)
	end,
...
OrderedTable = Class({
	__new = function(self, innerMetatable, ...)
		local args = {...}
		local contents = {}
		if type(innerMetatable) == "table" and getmetatable(innerMetatable) == Class then
			setmetatable(contents, innerMetatable)
		else
			table.insert(args, 1, innerMetatable)
		end
		
		local tbl = {__contents = contents, __kvpairs = {}}
		setmetatable(tbl, self)
		
		for _, v in ipairs(args) do
			if type(v) ~= "table" then error(tostring(v) .. "is not a key-value pair.") end
			tbl[v[1]] = v[2]
		end
	...
	Insert = function(self, index, key, value)
		if index < 1 or index > #self.__kvpairs + 1 then
			error("Index \"" .. tostring(index) .. "\" is out of range.")
		end
		
		local pair = ((value ~= nil or type(key) ~= "table" or #key ~= 2) and {key, value} or key)
		key, value = table.unpack(pair)
		...
		table.insert(self.__kvpairs, index, pair)
	end,
	
	__index = function(tbl, key)
		return tbl.__contents[key] or getmetatable(tbl)[key]
	end,
	
	__newindex = function(tbl, key, value)
		if tbl.__contents[key] ~= nil and value ~= nil then
			for i, v in ipairs(tbl.__kvpairs) do
				if v[1] == key then
					v[2] = value
					break
				end
		...
		elseif value ~= nil then
			table.insert(tbl.__kvpairs, {key, value})
		else
			local keyIndex = -1
			for i, v in ipairs(tbl.__kvpairs) do
				if v[1] == key then
					table.remove(tbl.__kvpairs, keyIndex)
					break
				end
			end
		end
		tbl.__contents[key] = value
	end,
	
	__pairs = function(tbl)
		local i = 1
		local function iterator(tbl, _)
			local pair = tbl.__kvpairs[i]
...
NumberedKeyTable = Class({
	...
	__index = function(tbl, key)
		local name, number = splitNumberedName(key)
		if number == nil and type(name) == "string" then
			return tbl[key .. "_1"]
		end
	end
})
...
function dataToCorrectType(data)
	if type(data) ~= "string" then
		return data
	end
	
	local dataAsNumber = tonumber(data)
	if dataAsNumber ~= nil then
		return dataAsNumber
	elseif data:lower() == "true" then
		return true
	elseif data:lower() == "false" then
		return false
	else
		return data
	end
...
function onlyWhitespace(str)
	return ((str:match("^%s*$")) and true or false) -- don't ask, I don't know either
end

function splitNumberedName(numberedName)
	if type(numberedName) ~= "string" then return numberedName end
	local lastUnderscore = numberedName:find("_[^_]*$") -- again, don't ask
	if lastUnderscore == nil then return numberedName end
	
	local name = numberedName:sub(1, lastUnderscore - 1)
	local number = tonumber(numberedName:sub(lastUnderscore + 1))
	
	return name, number
end
function cloneElement(element)
	return mod.xml.parse(mod.xml.stringify(element))
end

function shallowCopy(baseTable, copyTable)
	for k, v in pairs(baseTable) do
		copyTable[k] = v
	end
end

function getDeepCopy(baseTable)
	local metatable = getmetatable(baseTable)
	local copyTable = ((type(metatable) == "table") and ((getmetatable(metatable) == Class) and metatable() or setmetatable({}, metatable)) or {})
	assert(getmetatable(baseTable) == getmetatable(copyTable))
	
	for k, v in pairs(baseTable) do
		if type(v) ~= "table" then
			copyTable[k] = v
		else
			copyTable[k] = getDeepCopy(v)
		end
	end
...
function elementToTable(element)
	local elementTable = {prefix = element.prefix, name = element.name, attrs = OrderedTable(), content = OrderedTable(NumberedKeyTable)}
	for attrName, attrValue in element:rawattrs() do
		elementTable.attrs[attrName] = attrValue
	end
	
	local postFixNums = {}
	for node in element:childNodes() do
		node = node:as("text") or node:as("element")
		if node.type == "text" then
			table.insert(elementTable.content, node.content)
		else
			local keyName = node.name
			local postFixNum = postFixNums[keyName] or 1
			postFixNums[keyName] = postFixNum + 1
			keyName = keyName .. "_" .. tostring(postFixNum)
			--print(elementTable.content)
			elementTable.content[keyName] = elementToTable(node)
		end
	end
	
	if #elementTable.content.__kvpairs == 1 and type(elementTable.content.__kvpairs[1][2]) ~= "table" then
		elementTable.content = elementTable.content.__kvpairs[1][2]
	end
	
	return elementTable
end

function tableToElement(elementTable)
	local element = ((elementTable.prefix == nil or elementTable.prefix == "") and mod.xml.element(elementTable.name, elementTable.attrs.__contents) or mod.xml.element(elementTable.prefix, elementTable.name, elementTable.attrs.__contents))
	
	if type(elementTable.content) ~= "table" then
		element:append(elementTable.content)
		return element
	end
	
	for _, node in pairs(elementTable.content) do
		node = (type(node) ~= "table" and node or tableToElement(node))
		element:append(node)
	end
...
function elementTableToContextTable(elementTable)
	if type(elementTable.content) ~= "table" then return dataToCorrectType(elementTable.content) end
	
	local contextTable = {}
	for attrName, attrValue in pairs(elementTable.attrs) do
		contextTable["@" .. attrName] = dataToCorrectType(attrValue)
	end
	
	local content = elementTable.content
	for numberedName, node in pairs(content) do
		local node = (type(node) ~= "table" and node or elementTableToContextTable(node))
		
		if type(numberedName) == "number" then
			if not onlyWhitespace(node) then contextTable[numberedName] = node end
		else
			local name, number = splitNumberedName(numberedName)
			if number == 1 then
				contextTable[name] = (content[name .. "_2"] == nil and node or {node})
			else
				contextTable[name][number] = node
			end
		end
	end
...
local cachedAmmoTables = {}
function generateWeapon(baseWeapon, ammo)
	local ammoName = ammo.attrs.name
	local generatedWeaponName = baseWeapon.attrs.name .. "_" .. ammoName
	local generatedWeaponTable = elementToTable(baseWeapon)
	
	if not cachedAmmoTables[ammoName] then
		cachedAmmoTables[ammoName] = elementToTable(ammo)
	end
	local ammoTable = getDeepCopy(cachedAmmoTables[ammoName])
	local contextTable = elementTableToContextTable(generatedWeaponTable)
	
	generatedWeaponTable.attrs.name = generatedWeaponName
	evaluateAmmoStats(ammoTable, contextTable)
	applyAmmoStats(generatedWeaponTable, ammoTable, true)
	contextTable[BASE] = nil
	handleSpecialTags(generatedWeaponTable, ammoTable, contextTable)
	
	return tableToElement(generatedWeaponTable)
end

function evaluateAmmoStats(ammoTable, contextTable, statNum)
	if SPECIAL_TAGS[ammoTable.name] and not ammoTable.attrs[LITERAL_NODE] then
		return
	end
	
	if type(ammoTable.content) ~= "table" then
		evaluateStat(ammoTable, contextTable, statNum)
		return
	end
	
	for numberedName, element in pairs(ammoTable.content) do
		if type(element) == "table" then
			local name, number = splitNumberedName(numberedName)
			if number == 1 and not ammoTable.content[name .. tostring(number + 1)] then
				number = nil
			end
			
			evaluateAmmoStats(element, contextTable, number)
		elseif not onlyWhitespace(element) then
			ammoTable[numberedName] = luaxp.evaluate(element, contextTable) or element
		end
	end
end

function evaluateStat(ammoStatElementTable, contextTable, statNum)
	if statNum then
		contextTable[BASE] = (contextTable[ammoStatElementTable.name] or {})[statNum]
	else
		contextTable[BASE] = contextTable[ammoStatElementTable.name]
	end
	
	if not ammoStatElementTable.attrs[LITERAL_TEXT] then
		local value, err = luaxp.evaluate(ammoStatElementTable.content, contextTable)
		if value ~= nil then
			ammoStatElementTable.content = value
		else
			print("Expression \"" .. tostring(ammoStatElementTable.content) .. "\" failed to be evaluated with error: \"" .. err.message .. "\". Treating content as literal. If this was intended, add attribute " .. LITERAL_TEXT .. "=\"true\" to the element or surround the content in double quotes.")
		end
	end
end

function applyAmmoStats(generatedWeaponTable, ammoTable, topLevel)
	if SPECIAL_TAGS[ammoTable.name] and not ammoTable.attrs[LITERAL_NODE] then
		return
	end
	
	if not topLevel then
		shallowCopy(ammoTable.attrs, generatedWeaponTable.attrs)
	end
	
	--[[
	if type(ammoTable.content) ~= "table" then
		generatedWeaponTable.content = evaluateStat(ammoTable, contextTable)
	end
	]]
	
	local generatedWeaponContent = generatedWeaponTable.content
	for name, element in pairs(ammoTable.content) do
		if (type(element.content) ~= "table" or generatedWeaponContent[name] == nil) and not (SPECIAL_TAGS[element.name] and not element.attrs[LITERAL_NODE]) then
			generatedWeaponContent[name] = element
		else
			applyAmmoStats(generatedWeaponContent[name], element, false)
		end
	end
end

function handleSpecialTags(generatedWeaponTable, ammoTable, contextTable)
	for numberedName, element in pairs(ammoTable.content) do
		if type(element) == "table" then
			local name, _ = splitNumberedName(numberedName)
			if SPECIAL_TAG_FUNCS[name] and not element.attrs[LITERAL_NODE] then
				SPECIAL_TAG_FUNCS[name](element, generatedWeaponTable, ammoTable, contextTable)
			end
		end
	end
end

for _, weapon in ipairs(ammoWeaponBlueprints) do
	for _, ammo in ipairs(ammoBlueprints) do
		document.root:append(generateWeapon(weapon, ammo))
	end
end